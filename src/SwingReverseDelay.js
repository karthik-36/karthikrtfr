...
export default function Model({ ...props }) {
  const group = useRef()
  const { nodes, materials, animations } = useGLTF('/swingReverseDelay.glb')
  const { actions } = useAnimations(animations, group)

  
  useEffect(() => {
    actions['Armature|mixamo.com|Layer0'].play(0.5);
     //actions['Armature|mixamo.com|Layer0'].play();
  });
  return (
    <group scale = {[1.7,1.7,1.7]} rotation = {[0,0.1,0]} position = {[0,0,-5]}  ref={group} {...props} dispose={null}>
      <group name="Scene">
        <group name="Armature">
...
useGLTF.preload('/swingReverseDelay.glb')
